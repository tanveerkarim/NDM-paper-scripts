...
for i, mag in enumerate([g, r, z]):
    for l in [1, 0]: # Matched and unamtched
        for fnum in [2, 3, 4]:
            ...
            #Added following lines to remove NaN values from mag[ibool]
            mag_ibool = mag[ibool]
            mag_ibool = mag_ibool[~np.isnan(mag_ibool)]
            #
            ax_list[i, fnum-2].hist(mag_ibool, bins=mag_bins, color=colors[l], histtype="step", alpha=1, lw=2.5, label=labels[l], normed=True)
