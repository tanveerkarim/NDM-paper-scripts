...
for i, fnum in enumerate([2, 3, 4]):
    ...
    ra, dec, tycho, B, R, I, cn, w = load_DEEP2(dir_derived + ("deep2-f%d-photo-redz-oii.fits" % fnum))
    ibool = (tycho == 0)
    print("Apply Tycho-2 mask.\n")
    ra_d2, dec_d2, tycho, B, R, I, cn, w = load_DEEP2(dir_derived + ("deep2-f%d-photo-redz-oii.fits" % fnum), ibool=ibool)
    deep2 = load_fits_table(dir_derived + ("deep2-f%d-photo-redz-oii.fits" % fnum))[ibool]
    ...
    save_fits(trac_new, dir_derived + "DR5-matched-to-DEEP2-f%d-glim24p25.fits" % fnum)
    print("Completed.\n")

    print("Comparison of number of objects before and after save.")
    nobjs = trac_new.size
    trac_new = load_fits_table(dir_derived + "DR5-matched-to-DEEP2-f%d-glim24p25.fits" % fnum)
